        ...
        callback: (message: Message, responses: any[]) => {
            ...
            if (message.attachments.size > 0) {
                ...
                for (let i = 0; i < docs.length; i++) {
                    if (docs[i].match("/https:\/\/docs\.google\.com\/document\/d\/(.*?)\/.*?\?usp=sharing/")) return { name: "error", value: createErrorEmbed("Form Error", "If submitting links, you can only submit Google Document Links") };
                }
